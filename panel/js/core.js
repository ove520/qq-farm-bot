...
let accountLogFilterId = localStorage.getItem('accountLogFilterId') || 'all';
...
function updateFriendSubControlsState() {
    ...
    if (!wrap) return;
    ['auto-friend-steal', 'auto-friend-steal-skip-white-radish', 'auto-friend-help', 'auto-friend-bad'].forEach(id => {
        const input = $(id);
...
async function loadUserInfo() {
    try {
        const result = await api('/api/user/info');
        if (result) {
            currentUser = result;
            updateUserInfoUI();
        }
...
function updateUserInfoUI() {
    ...
    const usernameEl = $('user-username');
    if (usernameEl) usernameEl.textContent = currentUser.username || '-';
    
    const roleEl = $('user-role');
    if (roleEl) roleEl.textContent = currentUser.role === 'admin' ? '管理员' : '普通用户';
    
    const cardCodeEl = $('user-card-code');
    if (cardCodeEl) cardCodeEl.textContent = currentUser.card?.code || '-';
    
    const cardExpiryEl = $('user-card-expiry');
    const cardRemainingEl = $('user-card-remaining');
    
    if (currentUser.card) {
        if (currentUser.card.expiresAt) {
            const expiryDate = new Date(currentUser.card.expiresAt);
            if (cardExpiryEl) cardExpiryEl.textContent = expiryDate.toLocaleString();
            
            // 计算剩余时间
            const now = Date.now();
            const remaining = currentUser.card.expiresAt - now;
            if (remaining > 0) {
                const days = Math.floor(remaining / (24 * 60 * 60 * 1000));
                const hours = Math.floor((remaining % (24 * 60 * 60 * 1000)) / (60 * 60 * 1000));
                const minutes = Math.floor((remaining % (60 * 60 * 1000)) / (60 * 1000));
                if (cardRemainingEl) cardRemainingEl.textContent = `${days}天${hours}小时${minutes}分`;
            } else {
                if (cardRemainingEl) cardRemainingEl.textContent = '已过期';
            }
        } else {
            if (cardExpiryEl) cardExpiryEl.textContent = '永久';
            if (cardRemainingEl) cardRemainingEl.textContent = '无限期';
        }
    } else {
        if (cardExpiryEl) cardExpiryEl.textContent = '-';
        if (cardRemainingEl) cardRemainingEl.textContent = '-';
    }
...
function doRenew() {
    const modal = $('modal-renew');
    if (modal) {
        const errorEl = $('renew-error');
        if (errorEl) errorEl.textContent = '';
        const cardInput = $('renew-card-code');
...
async function doLogin() {
    ...
    const rememberUsername = $('remember-username')?.checked;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.modal').forEach(modal => {
        modal.addEventListener('click', (e) => {
            if (e.target === modal) {
                modal.classList.remove('show');
                if (modal.id === 'modal-alert' && alertResolve) {
                    alertResolve();
                    alertResolve = null;
                }
