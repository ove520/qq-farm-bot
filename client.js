...
async function triggerOfflineReminder(payload = {}) {
    try {
        ...
        if (!accountId) {
            log('系统', '无法发送下线提醒：未提供账号ID');
            return;
        }
        
        const accountsData = getAccounts();
        const account = accountsData.accounts.find(a => String(a.id) === accountId);
        if (!account) {
            log('系统', `无法发送下线提醒：找不到账号 ${accountId}`);
            return;
        }
        
        const username = account.username || '';
        if (!username) {
            log('系统', `无法发送下线提醒：账号 ${account.name} 未关联用户`);
            return;
        }
        
        // 根据用户名获取对应用户的下线提醒配置（必须用户隔离）
        const cfg = store.getOfflineReminder ? store.getOfflineReminder(username) : null;
        if (!cfg) {
            log('系统', `账号 ${account.name} 的下线提醒未配置（用户：${username}）`);
            return;
        }
...
function applyReloginCode({ accountId = '', accountName = '', authCode = '', uin = '' }) {
    ...
    if (found) {
        addOrUpdateAccount({
            ...
            username: found.username || '', // 保留原有用户名
        });
        restartWorker({
            ...found,
            code,
