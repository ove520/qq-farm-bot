                ...
                if (status) {
                    if (result.data.status === 'Wait') {
                        ...
                        try {
                            ...
                            console.log('[扫码登录] 保存账号, payload:', payload);
                            console.log('[扫码登录] adminToken:', adminToken ? '已设置' : '未设置');
                            const saveResult = await fetch('/api/accounts', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json', 'x-admin-token': adminToken },
                                body: JSON.stringify(payload)
                            }).then(r => r.json());
                            console.log('[扫码登录] 保存结果:', saveResult);

                            if (saveResult.ok) {
                                status.textContent = '✓ 保存成功';
                                setTimeout(() => {
                                    const modalEl = $('modal-add-acc');
...
$('btn-save-acc').addEventListener('click', async () => {
    ...
    console.log('[手动添加] 保存账号, payload:', payload);
    const result = await api('/api/accounts', 'POST', payload);
    console.log('[手动添加] 保存结果:', result);
