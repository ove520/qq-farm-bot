...
const { isAutomationOn, getFriendQuietHours, getStealFilterConfig, getStealFriendFilterConfig } = require('./store');
...
function analyzeFriendLands(lands, myGid, friendName = '', options = {}) {
    const { stealFilter = null } = options;
    const result = {
        stealable: [],   // 可偷
    ...
    for (const land of lands) {
        ...
        if (phaseVal === PlantPhase.MATURE) {
            if (plant.stealable) {
                ...
                // 偷菜过滤逻辑
                if (stealFilter && stealFilter.enabled) {
                    const mode = stealFilter.mode || 'blacklist';
                    const plantIds = stealFilter.plantIds || [];
                    const inList = plantIds.includes(plantId);
                    // 黑名单模式：在列表中则跳过；白名单模式：不在列表中则跳过
                    if ((mode === 'blacklist' && inList) || (mode === 'whitelist' && !inList)) {
                        if (showDebug) {
                            console.log(`  [${friendName}] 土地#${id}: ${plantName}(${plantId}) 被过滤跳过`);
                        }
                        continue;
                    }
                }
...
async function doFriendOperation(friendGid, opType) {
    ...
    try {
        const lands = enterReply.lands || [];
        const state = getUserState();
        const stealFilter = getStealFilterConfig();
        const status = analyzeFriendLands(lands, state.gid, '', { stealFilter });
        let count = 0;
...
async function visitFriend(friend, totalActions, myGid) {
    ...
    // 检查好友黑名单/白名单
    const stealFriendFilter = getStealFriendFilterConfig();
    if (stealFriendFilter && stealFriendFilter.enabled) {
        ...
        if ((mode === 'blacklist' && inList) || (mode === 'whitelist' && !inList)) {
            if (showDebug) {
                console.log(`  [${name}] 被好友过滤规则跳过 (模式: ${mode}, 在列表中: ${inList})`);
            }
            return;
        }
    }
    ...
    const stealFilter = getStealFilterConfig();
    const status = analyzeFriendLands(lands, myGid, name, { stealFilter });
